...
import { Search, Phone, MapPin, Package, Clock, User, Hash, Printer, Receipt } from "lucide-react";
...
export default function PackageList() {
  ...
  const handlePrintReceipt = (pkg: Delivery) => {
    ...
    toast({
      ...
      description: `Hóa đơn cho kiện ${pkg.orderNumber} đã được in`,
      variant: "default"
    });
  };

  const handlePrintSticker = (pkg: Delivery) => {
    ...
    toast({
      ...
      description: `Tem cho kiện ${pkg.orderNumber} đã được in`,
      variant: "default"
    });
  };

  const generateReceiptContent = (pkg: Delivery) => {
    return `
      <!DOCTYPE html>
      <html>
      <head>
        <title>Hóa Đơn - ${pkg.orderNumber}</title>
        <style>
          body { 
        ...
        <div class="receipt">
          ...
          <div class="section">
            ...
            <div class="row">
              ...
              <span>${getStatusInfo(pkg.status).label}</span>
            </div>
          </div>
          ...
          <div class="section">
            ...
            <div>${pkg.customerPhone || 'Không có SĐT'}</div>
            <div>${pkg.address}</div>
          </div>
          ...
          <div class="section">
            ...
            <div class="row">
              ...
              <span>${formatCurrency(pkg.codAmount)}</span>
            </div>
            ` : ''}
          </div>
  ...
  const generateStickerContent = (pkg: Delivery) => {
    return `
      <!DOCTYPE html>
      <html>
      <head>
        <title>Tem Dán Kiện - ${pkg.orderNumber}</title>
        <style>
          body { 
            font-family: Arial, sans-serif; 
            font-size: 11px; 
      ...
      <body>
        <div class="sticker">
          ...
          <div class="tracking">${pkg.orderNumber}</div>
          ...
          <div class="section">
            ...
            <div>Trạng thái: ${getStatusInfo(pkg.status).label}</div>
            <div>Dự kiến: ${pkg.estimatedTime}</div>
            ${pkg.codAmount > 0 ? `<div>COD: ${formatCurrency(pkg.codAmount)}</div>` : ''}
          </div>
        ...
        <div className="space-y-3">
          {filteredPackages.map((pkg) => {
            ...
            return (
              <div 
                key={pkg.id}
                className="p-4 bg-background border rounded-lg hover:bg-muted/30 transition-colors"
              >
                <div className="space-y-2">
                  ...
                  {/* Basic Info - Click to open modal */}
                  <div 
            ...
            <div className="bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="p-3 bg-blue-50 rounded-lg">
                  <h4 className="font-medium text-blue-800 mb-3">In ấn</h4>
                  <div className="grid grid-cols-2 gap-2">
                    <Button 
                      size="sm" 
                      variant="outline"
                      className="border-blue-300 text-blue-700 hover:bg-blue-100"
                      onClick={() => handlePrintReceipt(selectedPackage)}
                    >
                    ...
                    <Button 
                      ...
                      onClick={() => handlePrintSticker(selectedPackage)}
                    >
