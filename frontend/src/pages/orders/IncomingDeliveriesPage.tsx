...
import { Card, Table, PageHeader, Badge } from "../../components/ui";
...
export function IncomingDeliveriesPage() {
  ...
  const fetchDeliveries = async () => {
    ...
    try {
      ...
      const response = await orderService.getIncomingDeliveriesByCustomerId(
        user.id,
        {
          page: 0,
          size: 100,
      ...
      if (response && response.content) {
        // Chỉ lọc những đơn hàng đang giao (OUT_FOR_DELIVERY)
        const outForDeliveryOrders = response.content.filter(
          (order: Order) => order.status === "OUT_FOR_DELIVERY",
        );
        setDeliveries(outForDeliveryOrders);
      }
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge variant={map[status] || "info"}>{labels[status] || status}</Badge>
    );
  };
              ...
              {isLoading ? (
                ...
                deliveries.map((delivery) => {
                  ...
                  if (delivery.status === "OUT_FOR_DELIVERY") {
                    estimatedDelivery.setHours(
                      estimatedDelivery.getHours() + 2,
                    );
                  } else {
