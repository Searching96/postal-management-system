...
export function HubAdminPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const orders = await import("../../services/orderService")
          .then((m) => m.orderService.getOrders({ size: 1 }))
          .catch(() => ({ totalElements: 0 }));
        setStats((prev) => ({
          ...prev,
          orderCount: (orders as any).totalElements.toString(),
        }));
      } catch (e) {
        console.error(e);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        setSuccess(
          `Đã đăng ký thành công Hub Admin: ${response.data.fullName}`
        );
        setFormData({
          fullName: "",
          phoneNumber: "",
  ...
  const systemStats = [
    ...
    {
      label: "Tổng bưu cục tỉnh",
      value: provinceOffices.length.toString(),
      icon: Building2,
      color: "bg-green-500",
    },
    {
      label: "Tình trạng Hub",
      value: "Ổn định",
      icon: Building2,
      color: "bg-indigo-500",
    },
    ...
    <div className="space-y-8 pb-8">
          ...
          <div className="flex flex-col justify-between h-full gap-4">
            <div className="mb-2">
              <p className="text-sm text-gray-500 mb-4">
                Cấp quyền cho quản trị viên mới để quản lý hoạt động bưu chính
                tại các vùng miền.
              </p>
              <Button
            ...
            <div className="border border-dashed border-gray-200 rounded-2xl p-8 text-center bg-gray-50/50">
              ...
              <p className="text-xs text-gray-400">
                Xem và quản lý tài khoản Hub Admin.
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card title="Các Bưu cục Tỉnh phụ trách">
          <div className="space-y-4 h-full">
            <p className="text-sm text-gray-500">
              Danh sách các bưu cục cấp Tỉnh đang hoạt động trong các vùng bạn
              quản lý.
            </p>
            <div className="max-h-[300px] overflow-y-auto space-y-3 pr-2">
              {isOfficesLoading ? (
                ...
                <div className="py-10 text-center text-gray-400 text-sm">
                  Chưa có dữ liệu bưu cục
                </div>
              ) : (
                provinceOffices.map((office) => (
                  <div
                    key={office.id}
                    className="flex items-center p-3 rounded-xl border border-gray-100 hover:border-primary-100 hover:bg-primary-50/10 transition-colors"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 truncate">
                        {office.provinceName || "Bưu cục cấp tỉnh"}
                      </p>
                    </div>
                    <div className="ml-4">
                      <Badge
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <p className="text-sm text-gray-500 mb-4">
            Vui lòng nhập đầy đủ thông tin để khởi tạo tài khoản quản trị cho
            khu vực.
          </p>
          ...
          <FormSelect
            ...
            onChange={(val) =>
              setFormData({ ...formData, regionId: val as number })
            }
            required
