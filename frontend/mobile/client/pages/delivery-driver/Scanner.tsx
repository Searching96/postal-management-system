...
export function Scanner() {
  ...
  const handleConfirm = () => {
    ...
    const operation = scanResult?.type === 'pickup' ? 'lấy hàng' : 
                     scanResult?.type === 'delivery' ? 'giao hàng' : 'báo cáo thất bại';
  ...
  const getOperationTitle = () => {
    switch(scanResult?.type) {
      case 'pickup': return 'Xác nhận đã lấy hàng';
      case 'delivery': return 'Xác nhận giao thành công';
      case 'failed': return 'Báo cáo giao thất bại';
      default: return 'QR Scanner';
    }
  };

  const getOperationIcon = () => {
    switch(scanResult?.type) {
      case 'pickup': return <Scan className="h-5 w-5 text-blue-500" />;
      case 'delivery': return <CheckCircle className="h-5 w-5 text-green-500" />;
      case 'failed': return <Clock className="h-5 w-5 text-red-500" />;
      default: return <Camera className="h-5 w-5" />;
    }
  };

  return (
    <DriverShell title={getOperationTitle()} userName="Nguyễn Văn A" role="Bưu tá">
        ...
        {!scanResult ? (
            ...
            <div className="w-64 h-64 mx-auto bg-gray-100 rounded-lg border-2 border-dashed flex items-center justify-center">
              {isScanning ? (
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">Nhấn để quét QR</p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-blue-50 rounded-lg border">
              ...
              <p className="text-sm text-muted-foreground">{scanResult.customerName}</p>
              <p className="text-xs text-muted-foreground">{scanResult.address}</p>
            </div>
              ...
              {!confirmationImage ? (
                <div className="border-2 border-dashed rounded-lg p-4 text-center">
                  {isTakingPhoto ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">Đang chụp ảnh...</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <ImageIcon className="h-8 w-8 mx-auto text-gray-400" />
                      <p className="text-sm text-muted-foreground">Chụp ảnh xác nhận giao hàng</p>
                      <Button 
                        type="button"
                        variant="outline"
                ...
                <div className="space-y-2">
                  <div className="relative bg-gray-100 rounded-lg p-4 text-center">
                    ...
                    <p className="text-sm text-green-600">Đã chụp ảnh xác nhận</p>
